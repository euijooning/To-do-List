...
// 유저가 값을 입력한다.
let missionInput = document.querySelector(".mission-input")
// In Progress / Completed 탭을 누르면, 언더바가 이동한다.
...
let category = 'all-missions'; //초기값
let filterList = []; 
let list = [];

let underScored = document.getElementById("tab-underscored");

addButton.addEventListener("mousedown", addMission);
...
function addMission() {
    ...
    let missionValue = missionInput.value;
    let mission = {
        id: generateRandomID(),
        content: missionInput.value,
        isComplete: false
    };
...
function render() {
    ...
    for (let i = 0; i < list.length; i++) {
        if (list[i].isComplete) {
          result += `<div class="missions completed-missions" id="${list[i].id}">
...
function filter(event) {
    if (event) {
        ...
        underScored.style.top = event.target.offsetTop + (event.target.offsetHeight - 4) + "px";
    ...
    } else if(category === "completed-missions") {
        // 완료된 리스크를 보여준다.
